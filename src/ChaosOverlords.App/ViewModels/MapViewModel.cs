...
public sealed class MapViewModel : ViewModelBase, IDisposable
{
    ...
    private static (IReadOnlyList<SectorTileViewModel> Tiles, Dictionary<string, SectorTileViewModel> Lookup) CreateTiles(
        ...
        IReadOnlyDictionary<string, SiteData> siteLookup)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(sector.Id))
            {
                continue;
            }
            ...
            if (!string.IsNullOrWhiteSpace(sector.SiteName) && siteLookup.TryGetValue(sector.SiteName, out var siteData))
            {
    ...
    private static (int Row, int Column) ParseCoordinates(string sectorId)
    {
        ...
        {
            throw new InvalidOperationException("Sector identifiers must contain a row letter followed by a column number.");
        }
        ...
        {
            throw new InvalidOperationException(string.Format(CultureInfo.CurrentCulture, "Sector '{0}' contains an invalid column identifier.", sectorId));
        }
    ...
    private SiteData? _currentSiteData;
        ...
        public string SiteTooltip => BuildSiteTooltip(_currentSiteData ?? _defaultSiteData, OwnerDisplay, _gangCount, _chaosProjection);

        public string IncomeDisplay => FormatAmount(_currentIncome ?? _defaultIncome);

        public string ToleranceDisplay => string.Format(CultureInfo.CurrentCulture, "Tol {0}", _currentTolerance ?? _defaultTolerance);
        ...
        private static string BuildSiteTooltip(SiteData? site, string ownerDisplay, int gangCount, int chaosProjection)
        {
            ...
            foreach (var (label, extractor) in SiteStatExtractors)
            {
                ...
                if (value != 0)
                {
                    statLines.Add(string.Format(CultureInfo.CurrentCulture, "{0}: {1}", label, value));
                }
            }

            if (statLines.Count == 0)
            {
                statLines.Add("No site modifiers.");
            }
