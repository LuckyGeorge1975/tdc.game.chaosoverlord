...
public sealed class FileTurnEventWriter : ITurnEventWriter, IDisposable
{
    ...
    public FileTurnEventWriter(ITurnEventLog eventLog, LoggingOptions options, ILogPathProvider pathProvider)
    {
        ...
        if (_options.Enabled)
        {
            ...
            _writer = new StreamWriter(new FileStream(_logFilePath, FileMode.Append, FileAccess.Write,
                FileShare.ReadWrite))
            {
                AutoFlush = true
            };
    ...
    private static string CreateLogFilePath(LoggingOptions options, ILogPathProvider pathProvider)
    {
        ...
        var fileName = string.Format(CultureInfo.InvariantCulture, "{0}-{1:yyyyMMdd_HHmmss}.log",
            options.FileNamePrefix ?? "turn", DateTimeOffset.Now);
        return Path.Combine(directory, fileName);
    }

    private void TryTrimOldLogs()
    {
        if (_options.MaxRetainedFiles <= 0) return;
        ...
        try
        {
            ...
            if (files.Count <= _options.MaxRetainedFiles) return;
    ...
    private static string FormatEventLine(TurnEvent entry)
    {
        ...
        return string.Format(CultureInfo.InvariantCulture, "[{0}] T{1} {2} {3}: {4}", ts, entry.TurnNumber, phase,
...
internal static class TurnEventWriterReflection
{
    ...
    {
        return typeof(TurnEventWriter)
            .GetMethod("FormatEconomyDescription",
    ...
    {
        return typeof(TurnEventWriter)
            .GetMethod("FormatActionDescription",
                BindingFlags.NonPublic | BindingFlags.Static)!
