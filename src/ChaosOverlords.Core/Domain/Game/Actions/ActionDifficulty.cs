...
public sealed class ActionDifficulty
{
        ...
        int automaticFailureThreshold = 100)
    {
        if (minimumChance < 0 || minimumChance > 100)
            throw new ArgumentOutOfRangeException(nameof(minimumChance), minimumChance,
                "Minimum chance must be between 0 and 100.");

        if (maximumChance < 0 || maximumChance > 100)
            throw new ArgumentOutOfRangeException(nameof(maximumChance), maximumChance,
                "Maximum chance must be between 0 and 100.");
        ...
        if (automaticSuccessThreshold < 1 || automaticSuccessThreshold > 100)
            throw new ArgumentOutOfRangeException(nameof(automaticSuccessThreshold), automaticSuccessThreshold,
                "Automatic success threshold must lie between 1 and 100.");

        if (automaticFailureThreshold < 1 || automaticFailureThreshold > 100)
            throw new ArgumentOutOfRangeException(nameof(automaticFailureThreshold), automaticFailureThreshold,
                "Automatic failure threshold must lie between 1 and 100.");

        if (automaticSuccessThreshold >= automaticFailureThreshold)
            throw new ArgumentException("Automatic success threshold must be lower than automatic failure threshold.",
                nameof(automaticSuccessThreshold));
