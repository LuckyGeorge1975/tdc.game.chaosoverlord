...
/// <summary>
///     Command contributing research points to a specified project during the instant sub-phase.
/// </summary>
public sealed record ResearchCommand(
    ...
    Guid GangId,
    int TurnNumber,
    string ProjectId) : PlayerCommand(CommandId, PlayerId, GangId, TurnNumber)
{
    public override PlayerCommandKind Kind => PlayerCommandKind.Research;

    public override CommandPhase Phase => CommandPhase.Instant;
}

/// <summary>
///     Command to fabricate (purchase) an item blueprint during the transaction sub-phase.
///     The item is added to the player's warehouse upon successful execution.
/// </summary>
public sealed record FabricateCommand(
    Guid CommandId,
    Guid PlayerId,
    Guid GangId,
    int TurnNumber,
    string ItemName) : PlayerCommand(CommandId, PlayerId, GangId, TurnNumber)
{
    public override PlayerCommandKind Kind => PlayerCommandKind.Fabricate;

    public override CommandPhase Phase => CommandPhase.Transaction;
}