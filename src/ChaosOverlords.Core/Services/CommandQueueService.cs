...
public sealed class CommandQueueService : ICommandQueueService
{
        ...
        int turnNumber)
    {
        ...
        if (string.IsNullOrWhiteSpace(targetSectorId))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidSector,
                "Target sector id must be provided.");
        ...
        if (gang.OwnerId != playerId)
            return Failure(gameState, playerId, CommandQueueRequestStatus.NotOwned,
                "Gang does not belong to the player.");
        ...
        if (string.Equals(gang.SectorId, targetSector.Id, StringComparison.OrdinalIgnoreCase))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Gang is already in the target sector.");

        if (!SectorGrid.AreAdjacent(gang.SectorId, targetSector.Id))
            return Failure(gameState, playerId, CommandQueueRequestStatus.NotAdjacent,
                "Target sector is not adjacent to the gang's current position.");

        // Capacity: max 6 of the player's gangs per sector
        var ownerGangCountInTarget = game.Gangs.Values.Count(g =>
            g.OwnerId == playerId && string.Equals(g.SectorId, targetSector.Id, StringComparison.OrdinalIgnoreCase));
        if (ownerGangCountInTarget >= 6)
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Target sector is full (max 6 of your gangs).");

        var command = new MoveCommand(Guid.NewGuid(), playerId, gangId, turnNumber, gang.SectorId, targetSector.Id);
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Move to {0} queued.", targetSector.Id));
    }
        ...
        int turnNumber)
    {
        ...
        if (!string.Equals(gang.SectorId, sector.Id, StringComparison.OrdinalIgnoreCase))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Gang must occupy the sector to attempt control.");

        var command = new ControlCommand(Guid.NewGuid(), playerId, gangId, turnNumber, sector.Id);
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Control attempt queued for {0}.", sector.Id));
    }
        ...
        int turnNumber)
    {
        ...
        if (!sector.GangIds.Contains(gang.Id) &&
            !string.Equals(gang.SectorId, sector.Id, StringComparison.OrdinalIgnoreCase))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Gang must occupy the sector to influence the site.");

        if (sector.ControllingPlayerId != playerId)
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Sector must be under your control to influence the site.");
        ...
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Influence queued for {0}.", sector.Id));
    }
        ...
        int turnNumber)
    {
        ...
        if (!string.Equals(gang.SectorId, sector.Id, StringComparison.OrdinalIgnoreCase))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction,
                "Gang must occupy the sector to project chaos.");
        ...
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Chaos projection queued for {0}.", sector.Id));
    }
        ...
        int turnNumber)
    {
        ...
        // No board validation; research is an instant off-board contribution.
        var command = new ResearchCommand(Guid.NewGuid(), playerId, gangId, turnNumber, projectId);
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Research queued: {0}.", projectId));
    }
        ...
        int turnNumber)
    {
        ...
        if (string.IsNullOrWhiteSpace(itemName))
            return Failure(gameState, playerId, CommandQueueRequestStatus.InvalidAction, "Item name must be provided.");
        ...
        var command = new FabricateCommand(Guid.NewGuid(), playerId, gangId, turnNumber, itemName);
        return EnqueueCommand(gameState, command,
            string.Format(CultureInfo.CurrentCulture, "Fabricate queued: {0}.", itemName));
    }
    ...
    public CommandQueueResult Remove(GameState gameState, Guid playerId, Guid gangId, int turnNumber)
    {
        ...
        {
            CommandQueueMutationStatus.Removed => new CommandQueueResult(CommandQueueRequestStatus.Removed,
                "Command removed.", snapshot, null, mutation.ReplacedCommand),
            CommandQueueMutationStatus.NotFound => new CommandQueueResult(CommandQueueRequestStatus.NotFound,
                "No command queued for the specified gang.", snapshot),
            _ => new CommandQueueResult(CommandQueueRequestStatus.NotFound, "No command queued for the specified gang.",
                snapshot)
        };
    }
    ...
    private static void ValidateArguments(GameState gameState, Guid playerId, Guid gangId, int turnNumber)
    {
        ...
        if (gangId == Guid.Empty) throw new ArgumentException("Gang id must be provided.", nameof(gangId));
    ...
    private CommandQueueResult EnqueueCommand(GameState gameState, PlayerCommand command, string successMessage)
    {
        ...
        var message = mutation.Status switch
        {
            ...
            CommandQueueMutationStatus.Replaced => string.Format(CultureInfo.CurrentCulture, "Command updated: {0}",
                successMessage),
    ...
    private CommandQueueResult Failure(GameState gameState, Guid playerId, CommandQueueRequestStatus status,
        string message)
    {
    ...
    private CommandQueueSnapshot CreateSnapshot(GameState gameState, CommandQueue? queue, Guid playerId)
    {
        ...
        if (queue is null) return new CommandQueueSnapshot(playerId, Array.Empty<PlayerCommandSnapshot>());
    ...
    private PlayerCommandSnapshot CreateEntry(GameState gameState, PlayerCommand command)
    {
        ...
        return command switch
        {
            MoveCommand move => new PlayerCommandSnapshot(
                ...
                0,
                move.TurnNumber,
                string.Format(CultureInfo.CurrentCulture, "Move from {0} to {1}", move.SourceSectorId,
                    move.TargetSectorId)),
            ...
            ChaosCommand chaos => new PlayerCommandSnapshot(
                ...
                chaos.TurnNumber,
                string.Format(CultureInfo.CurrentCulture, "Chaos projection {0} in {1}", chaos.ProjectedChaos,
                    chaos.SectorId)),
            ResearchCommand research => new PlayerCommandSnapshot(
                research.CommandId,
                research.GangId,
                gangName,
                research.Kind,
                ...
                0,
                research.TurnNumber,
                string.Format(CultureInfo.CurrentCulture, "Research {0}", research.ProjectId)),
            FabricateCommand fabricate => new PlayerCommandSnapshot(
                fabricate.CommandId,
                fabricate.GangId,
                gangName,
                fabricate.Kind,
                ...
                null,
                0,
                fabricate.TurnNumber,
                string.Format(CultureInfo.CurrentCulture, "Fabricate {0}", fabricate.ItemName)),
